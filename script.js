...
function openProductDetail(id){
  ...
  product.images.forEach(img=>{
    const thumb=document.createElement("img"); thumb.src=img; thumb.onclick=()=>document.getElementById("detail-main-photo").src=img;
...
async function submitSignup(){
  ...
  alert("Signed up successfully!"); closeSignupForm();
}
...
async function submitLogin(){
  ...
  if(error){ alert(error.message); return; }
  alert("Logged in successfully!"); closeLoginForm();
}
...
async function showProfile(){
  ...
  if(!user){
    profileDiv.innerHTML=`<button onclick="openSignupForm()">Sign Up</button><button onclick="openLoginForm()">Log In</button>`;
    return;
  }
...
async function showCart(){
  ...
  let cartDiv=document.getElementById("cart-items"); cartDiv.innerHTML=""; let total=0;
  cartItems.forEach(item=>{ const product=products[item.product_id]; total+=product.price*item.quantity; cartDiv.innerHTML+=`<p>${product.name} x ${item.quantity} = AED ${product.price*item.quantity}</p>`; });
...
async function showSaves(){
  ...
  let savesDiv=document.getElementById("saves-items"); savesDiv.innerHTML="";
  savedItems.forEach(item=>{ const product=products[item.product_id]; savesDiv.innerHTML+=`<p>${product.name}</p>`; });
}
